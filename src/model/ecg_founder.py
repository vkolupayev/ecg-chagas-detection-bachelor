...
class DemographicEncoder(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.partial_features = out_features // 4
        self.fc1 = nn.Linear(self.in_features, self.partial_features)
        self.activation = Swish()
        self.fc2 = nn.Linear(self.partial_features, self.out_features)
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.einsum(
            "abc,ab->abc", out, se
        )  # broadcasting, equivalent to out = out * se.unsqueeze(-1)
        ...
        # TODO: consider adding normalization or an activation here
...
class BasicStage(nn.Module):
    ...
    def forward(self, x):
        ...
        for i_block in range(self.m_blocks):
            ...
            if self.verbose:
                logger.info(
                    f"stage: {self.i_stage}, block: {i_block}, in_channels: {net.in_channels}, out_channels: {net.out_channels}, outshape: {list(out.shape)}"
                )
                logger.info(
                    f"stage: {self.i_stage}, block: {i_block}, conv1: {net.conv1.in_channels}->{net.conv1.out_channels} k={net.conv1.kernel_size} s={net.conv1.stride} g={net.conv1.groups}"
                )
                logger.info(
                    f"stage: {self.i_stage}, block: {i_block}, convk: {net.conv2.in_channels}->{net.conv2.out_channels} k={net.conv2.kernel_size} s={net.conv2.stride} g={net.conv2.groups}"
                )
                logger.info(
                    f"stage: {self.i_stage}, block: {i_block}, conv1: {net.conv3.in_channels}->{net.conv3.out_channels} k={net.conv3.kernel_size} s={net.conv3.stride} g={net.conv3.groups}"
                )
...
class Net1D(nn.Module):
    ...
    def __init__(
        ...
        self.use_do = use_do
        self.do_prob = do_prob
        self.use_demo_encoder = use_demo_encoder
        self.demo_feat_len = demo_feat_len
        ...
        if self.use_demo_encoder:
            self.demo_encoder = DemographicEncoder(
                in_features=self.demo_feat_len, out_features=in_channels
            )
    ...
    def forward(self, x, demo=None):
        ...
        if self.verbose:
            logger.info(
                f"first conv: in_channels: {self.first_conv.in_channels}, out_channels: {self.first_conv.out_channels}, kernel outshape: {list(out.shape)}"
            )
        ...
        if self.verbose:
            logger.info(
                f"dense: in_features: {self.dense.in_features}, out_features: {self.dense.out_features}, deep features: {list(out.shape)}, mean pooled deep features: {list(deep_features.shape)}"
            )

        if self.use_demo_encoder:
            demo_features = self.demo_encoder(demo)
            deep_features = deep_features + demo_features
